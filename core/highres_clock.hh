...
//
    ...
    class highres_clock;
    class highres_system_clock;

/// \cond internal

    class highres_clock_impl final {
    public:
        using base_steady_clock = std::chrono::steady_clock;
        using base_system_clock = std::chrono::system_clock;

        // we make the clock period 1 us to do precise rate calculation.
        ...
        using steady_time_point = std::chrono::time_point<highres_clock, steady_duration>;

        using system_rep = base_system_clock::rep;
        using system_duration = std::chrono::duration<system_rep, period>;
        using system_time_point = std::chrono::time_point<highres_system_clock, system_duration>;
    ...
    private:
        ...
        // The timer expires every 10 us.
        static constexpr std::chrono::microseconds _granularity{10};
...
///
/// \see \c highres_system_clock for a low-resolution clock which produces time points corresponding to system time.
///
    class highres_clock final {
    public:
        using rep = highres_clock_impl::steady_rep;
        using period = highres_clock_impl::period;
        using duration = highres_clock_impl::steady_duration;
        using time_point = highres_clock_impl::steady_time_point;
        ...
        static time_point now() {
            return highres_clock_impl::steady_now();
        }
    };
...
///
/// This clock has the same granularity as \c highres_clock, but it is not required to be monotonic and its time points
/// correspond to system time.
///
/// The primary benefit of this clock is that invoking \c now() is inexpensive compared to
/// \c std::chrono::system_clock::now().
///
    class highres_system_clock final {
    public:
        using rep = highres_clock_impl::system_rep;
        using period = highres_clock_impl::period;
        using duration = highres_clock_impl::system_duration;
        using time_point = highres_clock_impl::system_time_point;

        static constexpr bool is_steady = highres_clock_impl::base_system_clock::is_steady;

        ///
        /// \note Outside of a Seastar application, the result is undefined.
        ///
        static time_point now() {
            return highres_clock_impl::system_now();
        }
